...
Perc.pos.by.ident = function(object,features.use=NULL, Count.mat = NULL, ident.use=NULL, thresh.use=0,do.plot=TRUE,do.transpose=FALSE,max.val.perc=NULL, max.val.exp=NULL,cols.use=gray.colors(10),max.size=10,min.perc=0,norm.exp=NULL,...) {
    ...
    if (is.character(features.use)){
      ...
      for (l in 1:length(features.use)){
        ...
        if (length(genes) > 1){
            ...
            vec.perc = c(vec.perc, sum(x1[1,] > thresh.use & x1[2,] > thresh.use)/ncol(x1))
            cells.both = cells.in.cluster[apply(x1, 2, function(x) x[1] > thresh.use & x[2] > thresh.use)]
            
             if (length(cells.both)<2){
               vec.exp = c(vec.exp, 0)
             } else {
               vec.exp = c(vec.exp,mean(apply(x1[,cells.both],2, mean)))
             }
        } else {
  ...
  if (do.plot){
    ...
    if (!do.transpose){
      ...
      p=ggplot(ExpVal, aes(y = factor(gene),  x = factor(cluster))) + geom_point(aes(colour = nTrans,  size =percExp)) + 
        scale_color_gradient(low ="blue",   high = "red", limits=c(0, max(ExpVal$nTrans) ))+scale_size(range = c(1, max.size))+   theme_bw() +nogrid
      ...
      p=ggplot(ExpVal, aes(y = factor(cluster),  x = factor(gene))) + geom_point(aes(colour = nTrans,  size =percExp)) + 
        scale_color_gradient(low ="black",   high = "olivedrab2", limits=c( 0, max(ExpVal$nTrans) ))+scale_size(range = c(1, max.size))+   theme_bw() +nogrid
